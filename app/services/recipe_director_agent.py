"""Recipe-based Director Agent for selecting clips via semantic search.

This agent uses a visual script (recipe) to guide clip selection. For each slot
in the visual script, it performs semantic search to find matching clips.
"""
...
class RecipeDirectorAgentError(Exception):
    """Exception raised when Recipe Director Agent fails."""
...
class RecipeDirectorAgent:
    """
    Recipe-based Director Agent that selects clips via semantic search.

    This agent uses a visual script (generated from a recipe) to guide assembly:
    1. For each slot in the visual script, perform semantic search
    2. Select the best clip based on similarity scores and duration
    3. Handle gaps with B-Roll generation prompts or text slides
    4. Generate Remotion-compatible timeline payload
    """

    def __init__(self) -> None:
        """Initialize the Recipe Director Agent."""
    ...
    async def assemble(
        self,
        db: AsyncSession,
        input_data: RecipeDirectorAgentInput,
    ) -> DirectorAgentOutput:
        """
        ...
        Raises:
            RecipeDirectorAgentError: If assembly fails
        """
        try:
            # Fetch visual script
            ...
            logger.info(
                f"[RECIPE-DIRECTOR] Assembled payload for project {input_data.project_id}: "
            ...
            logger.error(f"[RECIPE-DIRECTOR] Assembly failed: {e}")
            raise RecipeDirectorAgentError(f"Assembly failed: {e}") from e
    ...
    async def _get_visual_script(self, db: AsyncSession, script_id: uuid.UUID) -> VisualScript:
        ...
        if not script:
            raise RecipeDirectorAgentError(f"Visual script not found: {script_id}")

        if not script.slots:
            raise RecipeDirectorAgentError(f"Visual script {script_id} has no slots")
    ...
    async def _get_project(self, db: AsyncSession, project_id: uuid.UUID) -> Project:
        ...
        if not project:
            raise RecipeDirectorAgentError(f"Project not found: {project_id}")
